function [cash,P_bid,bookvalues] = naivedumptradingstrategy(data, dt_imbalance_avg, num_bins, dt_price_chg, ticker)
...
%   the right direction is > 0.5
    ...
    log_name = sprintf('strategy_naive/naive_dump_trading_%s.log', datestr(now,'yyyymmdd_HHMMSS'));
    ...
    for timestep = 2 : length(t)
        ...
        if P_bid(1,B,IB_curr) > 0.5
            ...
            if asset > 0
                ...
                fprintf(fid, '[%d] {%d, %d, %d}. Sell back long %d at %.2f (%d). [%d, %.2f].\n', t(timestep), IB_prev, DS_prev, IB_curr, asset, price, price_time, 0, cash);
                asset = 0;
            end
        ...
        elseif P_bid(3,B,IB_curr) > 0.5
            ...
            % buy back all if asset < 0
            if asset < 0
                price = data.SellPrice(time_ctr,1)/10000;
                price_time = data.Event(time_ctr,1);
                cash = cash + asset*price;
                fprintf(fid, '[%d] {%d, %d, %d}. Buy back short %d at %.2f (%d). [%d, %.2f].\n', t(timestep), IB_prev, DS_prev, IB_curr, asset, price, price_time, 0, cash);
                asset = 0;
            end
    ...
    saveas(f, sprintf('strategy_naive/dump-fig-bookvals-%s.jpg',ticker));
end
