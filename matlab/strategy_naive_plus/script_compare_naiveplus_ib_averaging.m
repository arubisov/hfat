...
tickers = {'FARO','INTC','NTAP','ORCL','SMH'};
tickers = {'NTAP','ORCL','SMH'};
tickers = {'GOOG'};
...
%     ds = 1000 * ones(5,1);
    ...
    [~,~,~,midprices] = naivetradingstrategy(data, imb(1), bin(1), ds(1), ticker, 0, 0, 1);
    [cash,~,bookvalues] = naiveplustradingstrategy(data, imb(1), bin(1), ds(1), ticker, 0, 0, 1);
    ...
    for i = 2:5
        [cash,~,bookvalues] = naiveplustradingstrategy(data, imb(i), bin(i), ds(i), ticker, 0, 0, i);
    ...
    for file = 2 : numel(listing)
        ...
        [~,~,~,midprices] = naivetradingstrategy(data, imb(1), bin(1), ds(1), ticker, 0, early_close, 1);

        for i = 1:5
            [cash,~,bookvalues] = naiveplustradingstrategy(data, imb(i), bin(i), ds(i), ticker, 0, early_close, i);
            [bookvalues,~] = normalizebookvals(bookvalues,midprices);
    ...
    T1 = 9.5 * 3600000;
    T2 = 16 * 3600000;
    t1 = [T1 + imb(1) : imb(1) : T2]; 
    t2 = [T1 + imb(2) : imb(2) : T2]; 
    t3 = [T1 + imb(3) : imb(3) : T2]; 
    t4 = [T1 + imb(4) : imb(4) : T2]; 
    t5 = [T1 + imb(5) : imb(5) : T2]; 

    avg_naive_1 = nanmean(year_bookvals_naive(:,1:numel(t1),1),1);
    avg_naive_2 = nanmean(year_bookvals_naive(:,1:numel(t2),2),1);
    avg_naive_3 = nanmean(year_bookvals_naive(:,1:numel(t3),3),1);
    avg_naive_4 = nanmean(year_bookvals_naive(:,1:numel(t4),4),1);
    avg_naive_5 = nanmean(year_bookvals_naive(:,1:numel(t5),5),1);
    avg_mid = nanmean(year_midprices,1) - 1;

    f = figure(101);
    hold on
    curve_mid = plot(t1/3600000, avg_mid, 'k', 'LineWidth', 5);
    curve_avg_naive_1 = plot(t1/3600000, avg_naive_1, 'r', 'LineWidth', 3);
    curve_avg_naive_2 = plot(t2/3600000, avg_naive_2, 'b', 'LineWidth', 3);
    curve_avg_naive_3 = plot(t3/3600000, avg_naive_3, 'g', 'LineWidth', 3);
    curve_avg_naive_4 = plot(t4/3600000, avg_naive_4, 'c', 'LineWidth', 3);
    curve_avg_naive_5 = plot(t5/3600000, avg_naive_5, 'm', 'LineWidth', 3);
    hold off
    ...
    save(sprintf('strategy_naive_plus/compare-ibmthd-naive-plus-%s.mat',ticker));
    close(f);
end